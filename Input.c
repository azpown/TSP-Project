...
{     
  ...
  char ** ligne_ptr = NULL;
  size_t * taille_alloc = NULL; /* correspond a un entier signé.*/
  while((taille_ligne=getline(ligne_ptr, taille_alloc, file)) != EOF)/* EOF généralement égal a -1*/
  {
    ...
    {
      input.edge_weight_type=alloc_chaine(taille_ligne,20,*ligne_ptr);
      continue;
    }
